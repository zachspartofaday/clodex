...
/**
 * An in-band `error` event on an otherwise normally closed 2xx SSE stream.
...
/**
 * Observe the decoded SSE body without altering the bytes relayed downstream.
 * Returns a promise ONLY on the compressed path, where the native zlib
 * transform emits its final decoded bytes asynchronously — after the client has
 * already been served — so the caller must await it before classifying.
 */
...
function forwardToAdapter(
  ...
  return new Promise(resolve => {
    ...
    res.once('finish', () => {
      ...
      // On the compressed path the decoded tail can still be in flight once the
      // client has already been served every byte, so classify only after it.
