...
/**
 * Why an inference terminated unsuccessfully. `error_sse` is an in-band error
 * event on an otherwise normally closed 2xx stream; the timeout categories name
 * which local deadline fired; the remaining two separate an upstream-attributed
 * failure from a local adapter exception.
 */
...
/** Accept only correlation ids whose complete shape is bounded and non-secret-like. */
function safeDiagnosticRequestId(value: unknown): string | undefined {
 ...
 * events are discarded rather than passed through from the raw diagnostic, so
 * hashes, fingerprints, upstream ids and message text never reach this file.
 */
