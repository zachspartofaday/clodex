...
describe('createResponsesWebSocketFetch', () => {
  ...
  it('continues when non-PDF Read.pages was sanitized from the echoed arguments', async () => {
    ...
    const socket = lastSocket();
    socket.emit('open');
    socket.emit('message', Buffer.from(JSON.stringify({ type: 'response.created', response: { id: 'resp_pages' } })));
    socket.emit('message', Buffer.from(JSON.stringify({
      type: 'response.output_item.done', output_index: 0,
      item: { type: 'function_call', call_id: 'call_p', name: 'Read', arguments: '{"file_path":"file.ts","pages":"1"}' },
    })));
    socket.emit('message', Buffer.from(JSON.stringify({ type: 'response.completed', response: { id: 'resp_pages' } })));
    await readAll(first);

    const echoedCall = { type: 'function_call', call_id: 'call_p', name: 'Read', arguments: '{"file_path":"file.ts"}' };
